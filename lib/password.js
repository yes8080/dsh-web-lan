...
* an update that does not change the resolved value commits nothing). When the
* effective password actually changes, {@link state.invalidateSessions} (set
* by the gate) is called to kick every existing session — a leaked password
* stops working for everyone the moment it is replaced. Inert without the
* settings service.
* @param ctx - the plugin's cordis context.
* @param configPassword - row config / environment password (the initial value).
* @param state - mutable `{ password, invalidateSessions }` owned by the gate.
*/
export function bridgePasswordSettings(ctx, configPassword, state) {
	...
	ctx.inject(["settings"], (sctx) => {
		...
		let source = () => entry;
		let lastPassword = configPassword;
		let lastOverride = false;
		const apply = (nextSource) => {
			source = nextSource;
			const resolved = source();
			if (typeof resolved?.password === "string" && resolved.password !== "" && resolved.password !== lastPassword) {
				lastPassword = resolved.password;
				state.password = resolved.password;
				state.invalidateSessions?.(); // the shared password changed: log everyone out
			}
