...
/** Persist sessions (0600); failures degrade to in-memory-only sessions. */
function saveSessions(sessionFile, sessions) {
	if (sessionFile === void 0) return false;
	try {
		mkdirSync(dirname(sessionFile), { recursive: true });
		writeFileSync(sessionFile, JSON.stringify({ version: 1, sessions: Object.fromEntries(sessions) }), { mode: 0o600 });
		return true;
	} catch {
		return false;
	}
...
export function wrapServer(server, auth, options = {}) {
	...
	// Kicks every current session (clear + persist); the password bridge calls
	// this when the shared password changes, so leaked sessions die with it.
