...
//
// Registers the 局域网访问 card in 设置 → 插件 → 插件配置 (the
// `settings.plugin.item` slot, keyed by the host's `lan-access` namespace):
// a write-only password field (the literal never crosses a wire — the host
// redacts it and mirrors the override state in the non-secret
// `passwordOverride` field). All copy goes through the locale service
// (zh/en), so the card follows the General settings language.
//
// Served verbatim by client-modules (`/plugins/dsh-web-lan/client.js`): the
// loader-wrapper format — no top-level imports, React.createElement only.
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// ── card styles: only the structure the framework has no component for.
		// Buttons, the password input, and the badges reuse the framework
		// primitives (Button / Input / Pill), which the theme styles for every
		// mode — no hand-rolled button colors. ──
		const css = ".lanA_a_card{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-3);border-radius:12px;list-style:none;transition:border-color .16s,background .16s}.lanA_a_card:hover{border-color:var(--dsw-alias-label-dimmed)}.lanA_a_cardOpen{background:var(--dsw-alias-bg-layer-2);border-color:var(--dsw-alias-label-dimmed)}.lanA_a_header{appearance:none;width:100%;font:inherit;color:inherit;text-align:left;cursor:pointer;background:0 0;border:0;border-radius:12px;align-items:center;gap:12px;padding:14px 16px;display:flex}.lanA_a_headText{flex-direction:column;flex:1;gap:4px;min-width:0;display:flex}.lanA_a_name{color:var(--dsw-alias-label-primary);font-size:15px;font-weight:600;line-height:1.4}.lanA_a_description{color:var(--dsw-alias-label-tertiary);font-size:13px;line-height:1.5}.lanA_a_body{border-top:1px solid var(--dsw-alias-border-l2);margin:0 16px;padding-bottom:8px}.lanA_a_readOnly{color:var(--dsw-alias-label-tertiary);margin:12px 0 0;font-size:12px;line-height:1.5}.lanA_a_field{flex-direction:column;gap:6px;padding:12px 0;display:flex}.lanA_a_head{align-items:center;gap:8px;display:flex}.lanA_a_label{min-width:0;color:var(--dsw-alias-label-primary);flex:1;font-size:13px;font-weight:500;line-height:1.5}.lanA_a_badges{align-items:center;gap:8px;display:inline-flex}.lanA_a_hint{color:var(--dsw-alias-label-tertiary);margin:0;font-size:12px;line-height:1.5}.lanA_a_resetRow{padding-top:4px}.lanA_a_footer{border-top:1px solid var(--dsw-alias-border-l2);justify-content:flex-end;align-items:center;gap:8px;padding:12px 0 4px;display:flex}.lanA_a_failed{min-width:0;color:var(--dsw-alias-label-error);flex:1;margin:0;font-size:12px;line-height:1.5}.lanA_a_btn{border-radius:8px;height:auto;padding:5px 14px;font-size:13px;line-height:1.5}";
		...
		/** The literal never rides a wire (host redacts it); the host's
		* non-secret `passwordOverride` mirror tells the card whether the stored
		* section overrides the configured password. Blank save keeps the current. */
		var LanAccessCardController = class {
			...
			constructor(scope, api) {
				...
				// Republish on every change; a store frozen at construction
				// (status "loading") would keep the card rendering nothing.
				this.listeners.add(() => {
			...
			/** Write the staged password (host applies it immediately + persists). */
			async save() {
		...
		// ── card component ────────────────────────────────────────────────────
		/** Header discloses a write-only password control; save applies immediately. */
		function LanAccessCard(props) {
			...
			const primitives = _deepseek_ai_dsh_client_ui_primitives;
				...
				children: [
					react.createElement("button", {
						...
						children: [
							...
							state.saved ? react.createElement(primitives.Pill, { children: t("saved") }) : null
						]
					}),
					open ? react.createElement("div", { className: lanCss.body, children: [
						...
						react.createElement("div", { className: lanCss.field, children: [
							react.createElement("div", { className: lanCss.head, children: [
								...
								react.createElement("span", { className: lanCss.badges, children: [
									react.createElement(primitives.Pill, { active: state.override, children: state.override ? t("passwordSet") : t("passwordUnset") })
								] })
							] }),
						...
						state.override ? react.createElement("div", { className: lanCss.resetRow, children: [
							react.createElement(primitives.Button, { className: lanCss.btn, variant: "ghost", size: "sm", disabled, onClick: props.reset, children: t("resetToConfigured") })
						] }) : null,
						react.createElement("div", { className: lanCss.footer, children: [
							...
							react.createElement(primitives.Button, { className: lanCss.btn, variant: "outline", disabled: (!state.dirty && !state.failed) || state.saving, onClick: props.discard, children: t("discard") }),
							react.createElement(primitives.Button, { className: lanCss.btn, variant: "primary", disabled: blocked, onClick: props.save, children: t(state.saving ? "saving" : "save") })
						] })
					] }) : null
				]
			});
		...
		/**
		* Mount the LAN access card into 设置 → 插件 → 插件配置.
		* `decode` bypasses schema validation: the redacted value is an empty
		* object, which must still read as "ready". The scope re-reads itself on
		* settings/document-updated, so the badge/reset follow the mirror.
		*/
		function apply(ctx) {
